...
class Tesseract {
  ...
  Tesseract(this._size) : _maxShadow = _size * 2 {
    for (int i = 0; i < 16; i++)
      _v.add(Vector4((i + 1) % 4 > 1 ? _size : -_size, i % 4 > 1 ? _size : -_size, i % 8 > 3 ? _size : -_size,
          i % 16 > 7 ? _size : -_size));
  }
...
class MyAppState extends State<MyApp> {
  ...
  void _loadStrings() async =>
      _s = List<dynamic>.of(json.decode(await DefaultAssetBundle.of(context).loadString('text.json'))['_'])
  ...
  Widget _makeDot(String s) {
    ...
    return Container(
        width: _size,
        height: _size,
        decoration: BoxDecoration(color: Color.lerp(Colors.grey, Colors.black87, _i), shape: BoxShape.circle));
  }
}
...
class TesseractPainter extends CustomPainter {
  ...
  bool shouldRepaint(TesseractPainter oldDelegate) =>
      x != oldDelegate.x || w != oldDelegate.w || shadow != oldDelegate.shadow;
